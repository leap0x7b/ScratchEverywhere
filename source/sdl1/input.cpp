...
#ifdef __SWITCH__
        if (SDL_JoystickGetButton(controller, 1)) { // B on xbox, A on switch
            Input::buttonPress("A");
            anyKeyPressed = true;
        }
        if (SDL_JoystickGetButton(controller, 0)) { // A on xbox, B on switch
            Input::buttonPress("B");
            anyKeyPressed = true;
        }
        if (SDL_JoystickGetButton(controller, 3)) { // Y on xbox, X on switch
            Input::buttonPress("X");
            anyKeyPressed = true;
        }
        if (SDL_JoystickGetButton(controller, 2)) { // X on xbox, Y on switch
            Input::buttonPress("Y");
            anyKeyPressed = true;
        }
#else
